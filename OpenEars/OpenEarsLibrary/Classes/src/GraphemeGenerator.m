...
- (NSString *) convertGraphemes:(NSString *)phrase {
    ...
    for (item=relation_head(utt_relation(u,relname)); 
		 ...
		 item=item_next(item))
    {
		NSString *bufferString = [[NSString alloc] initWithCString:item_feat_string(item,"name") encoding:NSUTF8StringEncoding];
	...
	NSString *stringToReturn = [[[NSString alloc] initWithCString:destinationString encoding:NSUTF8StringEncoding]autorelease];
